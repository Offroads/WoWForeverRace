...
- The class scan floor (`Scanner:NextClassFloor`) starts at the probe's estimate, else at the bottom (2, or the board's `minLevel` once it is full), and bisects towards the lowest floor whose result still fits under the 50 row cap, bounded above by the highest level seen (a full result from at or above that level aims for the level cap instead: cut off results are arbitrary, so the highest level seen can lag far behind). Floors known to overflow / fit are remembered for `FLOOR_BOUND_TTL`. Player names contain a space (`"First Surname"`), never a `-`
