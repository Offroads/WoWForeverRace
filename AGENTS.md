...
- Group messages reach a handful of players and cost nothing, so the group is used freely: `Sync:ScheduleGroupSync` (BPING to `GROUP`, debounced 2s) runs on `GROUP_ROSTER_UPDATE`, once the login sync is done (`SetReady`, for a login or `/reload` inside a group) and every `Config.GroupSyncInterval` (`Sync:InitGroupTicker`); a ding push goes to `GROUP` right away next to the zone `YELL`
