...
- The session's first scan is an unfiltered probe (`2-60`, repeated while the leaderboard is empty). `Scanner:RecordProbe` keeps its match count, levels and classes; a complete probe rests every class, otherwise `Scanner:ProbeFloor` estimates per class where a scan fits under the row cap
